...
def parse_ranking_table(html: str) -> list[dict[str, str]]:
    ...
    for table in soup.find_all("table"):
        ...
        for tr in rows_html:
            ...
            if len(cells) >= 4:
                rows.append({headers[i] if i < len(headers) and headers[i] else f"column_{i+1}": value for i, value in enumerate(cells)})
        if rows:
...
def get_form_frame(page):
    ...
    for selector in ["#CybotCookiebotDialogBodyLevelButtonLevelOptinAllowAll", "button:has-text('Accept all')", "button:has-text('Allow all')"]:
        try:
...
def main() -> None:
    ...
    items = []
    diagnostics = {"year": YEAR, "captured_at": datetime.now(timezone.utc).isoformat(), "source": PARENT}
    ...
    with sync_playwright() as p:
        ...
        options = {clean(o.inner_text()): o.get_attribute("value") or "" for o in frame.locator("#rankings-rankinglistfilter option").all()}
        ...
        for course, course_label in [("LC", "P50"), ("SC", "P25")]:
            for gender_post, gender_file, gender_label in [("M", "M", "Masculino"), ("W", "F", "Femenino")]:
                entry = {"year": YEAR, "course": course, "course_label": course_label, "gender": gender_file, "gender_label": gender_label, "records": 0, "file": ""}
                try:
                    ...
                    frame.locator(f"input[name='Rankings[genderFilter]'][value='{gender_post}']").check()
                    frame.locator("#rankings-eventtypefilter").select_option("")
                    frame.locator("#rankings-classfilter").select_option("")

                    with page.expect_response(lambda r: "/sdms/web/rankings/swm" in r.url and r.request.method == "POST", timeout=120000) as response_info:
